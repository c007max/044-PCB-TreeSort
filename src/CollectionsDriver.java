...
{			
	public static void main(String args[]) 	
	{
		...
		TreeSet<PCB> tsPCB	= new TreeSet<PCB>();	//#0031 Create List QMemUsed
		...
		for (int ii = 0; ii < QREADY__T; ii++)
		{
			...
			tsPCB.add(pcb0) ;		//#0072 Add PCB object onto tsPCB
		}
		

		for (PCB loopI : QReady)
			System.out.printf("QReady:\t%s\n"	,loopI.showPCB()) ; 
		
		Iterator itr = tsPCB.iterator() ;
		while (itr.hasNext())
			System.out.printf("tsPCB:\t%s\n"	,itr.next());
		
		//System.out.printf("tsPCB \tmin: %d\tmax: %d\n"	,tsPCB.first()	,tsPCB.last()	);
		
		boolean Exists = tsPCB.contains(tsPCB.first()) ;
		System.out.printf("tsPCB max exists ? : " + Exists) ;
